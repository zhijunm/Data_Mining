...
library(neuralnet)
df <- read.csv("c:/Users/mazhi/Documents/R/560_data_mining/tinydata.csv")
...
accidents.df <- read.csv("c:/Users/mazhi/Documents/R/560_data_mining/accidentsnn.csv")
table(accidents.df$SUR_COND)
# selected variables
...
names(validData) <- c(vars, 
                   paste("SUR_COND_", c(1, 2, 3, 4), sep=""), paste("MAX_SEV_IR_", c(0, 1, 2), sep=""))
...
training.class <- as.factor(apply(training.prediction$net.result,1,which.max)-1)
confusionMatrix(training.class, as.factor(accidents.df[training,]$MAX_SEV_IR))

validation.prediction <- compute(nn, validData[,-c(8:11)])
validation.class <-as.factor(apply(validation.prediction$net.result,1,which.max)-1)
confusionMatrix(validation.class, as.factor(accidents.df[validation,]$MAX_SEV_IR))
